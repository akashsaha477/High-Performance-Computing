...
int main() {
    ...
    if (TF_GetCode(status) != TF_OK) {
        ...
        cerr << "Make sure you ran the Python script to create the SavedModel at: " << MODEL_DIR << endl;
        return 1;
    }
    ...
    if (!input_op.oper) {
        // fallback: try serving_default_keras_tensor too
        input_op = {TF_GraphOperationByName(graph, "serving_default_keras_tensor"), 0};
        if(!input_op.oper) {
            cerr << "Error: Failed to find Input Operation. Use saved_model_cli to inspect ops." << endl;
            return 1;
        }
    }
    ...
    for (int i = 0; i < NUM_TEST_SAMPLES; i += BATCH_SIZE) {
        ...
        TF_Tensor* input_tensor = TF_NewTensor(
            TF_FLOAT, input_dims, num_input_dims,
        ...
        if (!output_values[0]) {
            cerr << "Error: output tensor was NULL after session run." << endl;
            TF_DeleteTensor(input_tensor);
            continue;
        }
