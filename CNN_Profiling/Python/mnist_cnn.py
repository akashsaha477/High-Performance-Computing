...
EXPORT_DIR_BEST = os.path.join(BASE_DIR, "saved_mlp_best")   # SavedModel dir
EXPORT_DIR_LAST = os.path.join(BASE_DIR, "saved_mlp_last")   # SavedModel dir
EXPORT_FILE_BEST = EXPORT_DIR_BEST + ".keras"                # Keras single-file
EXPORT_FILE_LAST = EXPORT_DIR_LAST + ".keras"                # Keras single-file
...
t1 = now(); record("model-build", t1 - t0)
...
t1 = now(); record("input pipeline", t1 - t0)
...
t0 = now()
# Save as TensorFlow SavedModel (directory) - good for C/C++ TF C API
tf.saved_model.save(model, EXPORT_DIR_LAST)
# Also save a Keras single-file model for Python convenience
model.save(EXPORT_FILE_LAST)
...
if os.path.exists(best_weights_path):
    ...
    t0 = now()
    tf.saved_model.save(best, EXPORT_DIR_BEST)
    best.save(EXPORT_FILE_BEST)
...
t1 = now(); record("python inference(128)", t1 - t0)
